    ...
    <div class="card card-body">
        ...
        <div class="" v-for="(institucion, index) in institucionales" :key="`campos-${index}`">
            <div class="row col-sm-12">
                <b-form-input autocomplete="off" @input="buscarAutores(index)" @blur="borrarCamposBusqueda(index)"
                              v-model="institucion.value" class="col-sm-10" :name="`institucional-${index}`"
            ...
            <ul v-if="institucion.results.length > 0 && institucion.query" :id="`results-${index}`">
                <li @click="elegirResultado(index, result)" class="nav-item"
                    v-for="result in institucion.results.slice(0,10)" :key="result.id">
...
<script>
export default {
    name: 'registro-autores-institucionales',
    mounted() {
    },
    prop: {},
    data() {
