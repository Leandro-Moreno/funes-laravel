...
<div class="col-sm-12 row">
  ...
  <div class="" v-for="(input, index) in inputs" :key="`campos-${index}`">
    <div class="row col-sm-12">
        <b-form-input autocomplete="off" @input="onChange(index)" @blur="borrarCamposBusqueda(index)" v-model="input.valueApellido" class="col-sm-3" :name="`apellido-${index}`" placeholder=""></b-form-input>
        <b-form-input autocomplete="off" @input="onChange(index)" @blur="borrarCamposBusqueda(index)" v-model="input.valueNombre" class="col-sm-4" :name="`nombre-${index}`" placeholder=""></b-form-input>
        <b-form-input autocomplete="off" @input="onChange(index)" @blur="borrarCamposBusqueda(index)" v-model="input.valueMail" class="col-sm-3" :name="`mail-${index}`" placeholder="" type="email"></b-form-input>
        <div class="col-sm-1">
      ...
      <ul v-if="input.results.length > 0 && input.query" :id="`results-${index}`">
          <li @click="elegirResultado(index, result)" class="nav-item" v-for="result in input.results.slice(0,10)" :key="result.id">
